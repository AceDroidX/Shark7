    ...
    for (const field in updated) {
        ...
        switch (field) {
            ...
            case "cdata10":
                ...
                msg += `\n第${
                    parseInt(field.replace("cdata", "")) / 2 - 2
                }个徽章类型改变: ${badgeName}`;
                break;
            case "cdata7":
            case "cdata9":
            case "cdata11":
                msg += `\n第${
                    (parseInt(field.replace("cdata", "")) - 1) / 2 - 2
                }个徽章数据改变: ${updated[field]}`;
                break;
            case "cdata12":
            case "cdata14":
            case "cdata16":
                let tracerName = getKeyByValue(updated[field], TracerName);
                msg += `\n第${
                    parseInt(field.slice(-1)) / 2
                }个追踪器类型改变: ${tracerName}`;
                break;
            case "cdata13":
            case "cdata15":
            case "cdata17":
                msg += `\n第${
                    (parseInt(field.slice(-1)) - 1) / 2
                }个追踪器数据改变: ${updated[field]}`;
                break;
            case "cdata18":
